...
describe('Add supplier Functionality Tests', () => {
    ...
    it('should validate code with invalid name', () => {
      cy.fixture('supplier').then((data) => {
        const d = data.invalidName;
        ...
        cy.on('window:alert', (text) => {
          expect(text).to.include('Error: Wromg name format.');
        });
      });
    });

      it('should validate code with existing code', () => {
      cy.fixture('supplier').then((data) => {
        ...
        cy.on('window:alert', (text) => {
          expect(text).to.include('Error: Supplier with code "SUP004" already exists. Please use a unique code.');
        });
      });
    });


    it('should validate code with empty ingredient', () => {
      cy.fixture('supplier').then((data) => {
        const d = data.emptyIngredient;
        ...
        cy.get('#supplier-ingredient').then(($input) => {
    ...
    it('should validate measurement with empty inputs', () => {
      cy.fixture('supplier').then((data) => {

        cy.contains('Save').click();
        
        cy.get('#supplier-code').then(($input) => {
          expect($input[0].checkValidity()).to.be.false;
          expect($input[0].validationMessage).to.eq('Please fill out this field.');
        });
      });
